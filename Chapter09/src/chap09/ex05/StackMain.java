...
//		System.out.println(towel.getColor());
		...
		// 메서드 체이닝 - 앞 메서드에서 진행하여 반환된 값을 가지고 무언가 연결해서 진행 할 떄
...
//		System.out.println(box.pop().getColor());
		...
		// 다른 녀석들은 값을 불러온다고 해서 사라지지 않는다.
		// stack 은 pop() 을 사용하면 사라진다.
		while(!box.isEmpty()) {
			// peek() 은 확인만 하고 다시 집어 넣는다.
			System.out.println(box.pop().getColor() + "색 수건 꺼냄" + box.size() + " 장 남음");
		}
